...
export function Board({ tiles, selected, hintIds, matchIds, mistakeId, glowIds, onTileClick }) {
  ...
  var layout = useMemo(function() {
    if (tiles.length === 0) {
      return { minCol:0, minRow:0, rawW:0, rawH:0, sorted:[], scale:1 };
    }
    ...
    var sorted = tiles
      .filter(function(t) { return !t.removed; })
      .sort(function(a, b) {
  ...
  if (active.length === 0) {
    return (
      <div style={{ width:"100%", height:"100%", display:"flex", alignItems:"center", justifyContent:"center" }}>
        <div style={{ color:"#ff6b00", fontSize:40, fontWeight:900 }}>CLEAR!</div>
      </div>
    );
  }
